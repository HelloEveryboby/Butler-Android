...
def initialize(files_dir=None):
    ...
    # Start bridge server (protocol adaptation layer: HTTP+WS → call_plugin)
    try:
        from bridge_server import set_call_plugin, set_skill_manager, start as start_bridge
        set_call_plugin(call_plugin)
        set_skill_manager(skill_manager)
        start_bridge(host='127.0.0.1', port=8080)
